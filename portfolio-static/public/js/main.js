const projects = [
  ...
  },
  {
    ...
    description: "An online tutoring service project for students & tutors alike.",
...
function renderProjects() {
  track.innerHTML = projects.map(project => `
    <div class="bg-gray-700 p-4 rounded text-center flex-shrink-0" style="width: ${getCardWidth()}px">
      <img src="${project.img}" alt="${project.title}" class="w-full h-auto mx-auto rounded-md object-cover" loading="lazy" />
      <h3 class="text-xl font-bold mt-3">${project.title}</h3>
      <p class="text-gray-300 mt-2">${project.description}</p>
      <a href="${project.link}" target="_blank" class="mt-4 inline-block bg-yellow-500 text-black py-2 px-4 rounded-lg shadow-lg hover:bg-yellow-600 transition">Visit Website</a>
    </div>
  `).join('');
  updateTrackPosition();
}
...
document.getElementById('prev-project').addEventListener('click', () => {
  currentIndex = currentIndex === 0 ? projects.length - projectsPerPage : currentIndex - 1;
  updateTrackPosition();
});

document.getElementById('next-project').addEventListener('click', () => {
  currentIndex = currentIndex >= projects.length - projectsPerPage ? 0 : currentIndex + 1;
  updateTrackPosition();
});
